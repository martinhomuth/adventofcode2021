...
(defun nth-window (list n)
  (let* ((current-list (nthcdr (1- n) list)))
    (if (>= (list-length current-list) 3)
	(+ (car current-list) (cadr current-list) (caddr current-list))
	0)))
...
(defun list-window-measurement-increases ()
  (let ((numbers (get-input "input.txt"))
	(acc 0))
    (count-recursive numbers acc)))
